...
void
Java_cc_openframeworks_OFAndroid_onCustom() {
        if (androidApp) androidApp->onCustom();
    }
...
void Java_cc_openframeworks_OFAndroid_passInt(JNIEnv*  env, jobject  thiz, jint  myInt) {

	if (androidApp) {
		androidApp->onInt(myInt); // should be able to go through directly...
	}
}
